 ...
 * @since 1.0.0
 */
class Miraculous_track extends Widget_Base {
	...
	protected function register_controls() {
		...
		$this->start_controls_section(
            'heading',
            [
                'label' => esc_html__( 'Track', 'miraculous' ),
            ]
        );
        $this->add_control(
            'track_heading',
            [
                'label'       => esc_html__( 'Track Heading', 'miraculous' ),
        ...
        $this->add_control(
			...
			[
				...
				'options' => [
					...
					'abstyle2' => esc_html__( 'Weekly', 'miraculous' ),
					'abstyle3' => esc_html__( 'Grid', 'miraculous' ),
					'abstyle4' => esc_html__( 'Slider 2', 'miraculous' ),
					'abstyle5' => esc_html__( 'List', 'miraculous' ),
				],
			]
		);
		$this->add_control(
			...
			[
				...
				'options' => [
					...
					'premium' => esc_html__( 'Premium', 'miraculous' ),
				],
			]
		);
		$this->add_control(
			'track_filter',
			[
				'label' => esc_html__( 'Songs Filter By', 'miraculous' ),
				'type' => \Elementor\Controls_Manager::SELECT,
				'default' => 'all',
				'options' => [
					'select' => esc_html__( 'Select', 'miraculous' ),
					'song_views_count'  => esc_html__( 'Song View', 'miraculous' ),
					'song_dowenload_counter' => esc_html__( 'Song Download', 'miraculous' ),
				],
			]
		);
		$this->add_control(
			'track_type',
			[
				'label' => __( 'Track Type', 'miraculous' ),
                'label_block' => ('bool'),
                'type' => Controls_Manager::SELECT2,
                'multiple' => true,
                'options' => get_custom_type_category('music-type')
			]
		);
        $this->add_control(
            'track_number',
            [
                'label'       => esc_html__( 'Number of Track', 'miraculous' ),
                'type'        => Controls_Manager::TEXT,
                'label_block' => true,
                'default'     => esc_html__( '', 'miraculous' ),
            ]
        ...
        $this->add_control(
			'song_view',
			[
				'label' => esc_html__( 'Song View Counter', 'miraculous' ),
		...
		$this->add_control(
			'song_download',
			[
				'label' => esc_html__( 'Song View Dowenload', 'miraculous' ),
				'type' => Controls_Manager::SWITCHER,
				'no' => esc_html__( 'No', 'miraculous' ),
				'yes' => esc_html__( 'Yes', 'miraculous' ),
				'return_value' => 'yes',
	...
	protected function render() {
        ...
        if(empty($settings['track_type'])){
            $test = get_terms('music-type');
            $tests =  $test[0];
            $settings['track_type'] = $tests->term_id;
        }
        ...
        if($settings['track'] == 'all'){
            $track_type =  array ( 'free', 'premium' );
        }
        else{
            $track_type = $settings['track'];
        }
        if( is_user_logged_in() && $lang_data ){
            $sg_args = array('post_type' => 'ms-music',
                        'posts_per_page' => $settings['track_number'],
                        'meta_query' => array(
                            'relation' => 'OR',
                            'music_types' => array(
        ...
        <div class="ms_fea_album_slider">
            <?php if(!empty($settings['track_heading'])){ ?>
            <div class="ms_heading">
                <h1><?php echo esc_html( $settings['track_heading'] ); ?></h1>
            </div>
            <?php } ?>
        ...
        if( $settings['style_format'] == 'abstyle5' ): ?>
            <?php if(!empty($settings['track_heading'])){ ?>
            <h2 class="music_listwrap_tttl"><?php echo esc_html( $settings['track_heading'] ); ?></h2>
            <?php } ?>
                            ...
                            <div class="ms_songslist_inner">
                                <div class="ms_songslist_left">
                                    <div class="songslist_number play_btn play_music" data-musicid="<?php esc_html_e(get_the_id()); ?>" data-musictype="<?php printf($list_type); ?>">
                                        ...
                                        <span class="songslist_play"><img src="<?php echo esc_url($play_img); ?>" alt="Play" class="img-fluid"/></span>
                                    </div> 
