 ...
 * @since 1.0.0
 */
class Miraculous_radio extends Widget_Base {
	...
	protected function register_controls() {
		...
		$this->start_controls_section(
            'heading',
            [
                'label' => esc_html__( 'Radio', 'miraculous' ),
            ]
        );
        $this->add_control(
            'radio_heading',
            [
                'label'       => esc_html__( 'Radio Heading', 'miraculous' ),
		...
		$this->add_control(
			'radio_type',
			[
				'label' => __( 'Radio Type', 'miraculous' ),
                'label_block' => ('bool'),
                'type' => Controls_Manager::SELECT2,
                'multiple' => true,
                'options' => get_custom_type_category('radio-type')
			]
		);
		$this->add_control(
            'radio_slide',
            [
                'label'       => esc_html__( 'Per Slide Radio', 'miraculous' ),
                'type'        => Controls_Manager::TEXT,
                'label_block' => true,
                'default'     => esc_html__( '', 'miraculous' ),
            ]
        ...
        $this->add_control(
            'radio_number',
            [
                'label'       => esc_html__( 'Number of Radio', 'miraculous' ),
                'type'        => Controls_Manager::TEXT,
                'label_block' => true,
                'default'     => esc_html__( '', 'miraculous' ),
            ]
	...
	protected function render() {
        ...
        if(empty($settings['radio_type'])){
            $test = get_terms('radio-type');
            $tests =  $test[0];
            $settings['radio_type'] = $tests->term_id;
        }
        if( is_user_logged_in() && $lang_data ){
        
            $ar_args = array('post_type' => 'ms-radios',
                        'posts_per_page' => $settings['radio_number'],
                        'tax_query' => array(
                                array(
                                    'taxonomy' => 'radio-type',
                                    'terms' => $settings['radio_type']
                                ),
                                array(
                                    'taxonomy' => 'language',
                                    'terms' => $lang_data
        ...
        $ar_style = $settings['radio_type']['0'];
        if( $radio_posts->have_posts() ): 
          $slidermeta = array('per_view' => $settings['radio_slide']);
            if( $settings['style_format'] == 'abstyle1' ): ?>
                <div class="ms_fea_album_slider ms-slider">
                    <?php if(!empty($settings['radio_heading'])){ ?>
                    <div class="ms_heading">
                        <h1><?php echo esc_html( $settings['radio_heading'] ); ?></h1>
                    </div>
                    <?php } ?>
